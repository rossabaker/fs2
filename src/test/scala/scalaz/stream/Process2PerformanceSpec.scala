...
import scalaz.stream.Process2._


/**
 * Created by pach on 01/03/14.
 */
object Process2PerformanceSpec extends Properties("Process2-performance") {

  import TestUtil._

  object append {
    def left(n: Int): Process2[Task, Int] =
      (0 to n).map(emit).foldLeft(emit(0))(_ ++ _)

    def right(n: Int): Process2[Task, Int] =
  ...
  object worstCase {
    ...
    def churned(n: Int): Process2[Task, Int] = {
      @annotation.tailrec
  ...
  def associationCheck(
    left: Int => Process2[Task, Int]
    , right: Int => Process2[Task, Int]
